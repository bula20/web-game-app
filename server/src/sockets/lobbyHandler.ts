...
async function setActiveRoom(
  ...
  if (userId.startsWith("guest_")) {
    if (code && displayName)
      guestActiveRooms.set(userId, { code, displayName });
  ...
  try {
    ...
    /* ignorujemy: drobny zapis w bazie, nie blokujmy reszty */
  }
}
  ...
  socket.on(
    ...
    async (data: {
      ...
      try {
        ...
        socket.emit("user:active_room_changed", {
          code,
          gameType: data.gameType,
          status: "waiting",
        });
  ...
  socket.on("room:leave", async ({ code }: { code: string }) => {
    try {
      ...
      if (room.players.length === 0) {
        ...
        socket
          .to(`room:${code}`)
          .emit("room:player_left", { socketId: socket.id, userId });
        io.to(`lobby:${room.gameType}`).emit("lobby:room_updated", room);
      }
