...
export const GAME_DISCONNECT_GRACE = 20; 
export const LOBBY_NON_HOST_GRACE = 20; 
export const HOST_AWAY_TTL = 120; 


type GameDisconnectHandler = (
  io: Server,
  code: string,
  userId: string,
) => void | Promise<void>;
type GameReconnectHandler = (
  io: Server,
...
export function registerGameDisconnectHandler(
  gameType: GameType,
  handler: GameDisconnectHandler,
) {
  disconnectHandlers.set(gameType, handler);
}

export function registerGameReconnectHandler(
  gameType: GameType,
  handler: GameReconnectHandler,
) {
...
async function promoteNextHost(io: Server, code: string) {
  try {
    ...
    if (room.players.length === 0) {
      ...
      io.to(`lobby:${room.gameType}`).emit("lobby:room_removed", {
        roomId: room._id,
      });
      return;
    }
...
async function markPlayerDisconnected(
  ...
  try {
    ...
    room.disconnectedPlayers = room.disconnectedPlayers.filter((d) =>
      isGuest ? d.guestId !== userId : d.userId?.toString() !== userId,
    ) as any;
...
async function clearPlayerDisconnected(code: string, userId: string) {
  try {
    ...
    await Room.findOneAndUpdate(
      ...
      {
        $pull: {
          disconnectedPlayers: isGuest ? { guestId: userId } : { userId },
        },
      },
    );
  } catch {
...
export function setupPresenceHandler(io: Server, socket: AuthenticatedSocket) {
  ...
  handleReconnect(io, socket).catch((err) =>
      ...
      for (const room of rooms) {
        ...
        if (room.status === "waiting" || room.status === "host_away") {
          if (isHost) {
            ...
            io.to(`room:${room.code}`).emit("room:host_away", {
              expiresIn: HOST_AWAY_TTL,
            });
            io.to(`lobby:${room.gameType}`).emit("lobby:room_updated", room);
            ...
            setTimeout(async () => {
              ...
              if (handler) {
                try {
                  ...
                  console.error(
                    `Game disconnect handler error (${room.gameType}):`,
                    err,
                  );
                }
              }
              await clearPlayerDisconnected(code, userId);
              io.to(`room:${code}`).emit("player:reconnect_expired", {
                userId,
              });
...
async function handleReconnect(io: Server, socket: AuthenticatedSocket) {
  ...
  for (const room of rooms) {
    ...
    if (room.status === "host_away" && isOriginalHost) {
      ...
      socket
        .to(`room:${code}`)
        .emit("room:host_returned", { hostName: socket.displayName });
    }
    io.to(`lobby:${room.gameType}`).emit("lobby:room_updated", room);
    ...
    if (room.status === "in_progress") {
      ...
      if (handler) {
        try {
          ...
          console.error(
            `Game reconnect handler error (${room.gameType}):`,
            err,
          );
        }
      }
