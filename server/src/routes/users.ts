...
router.put('/me/avatar', authMiddleware, async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthRequest;
    if (authReq.isGuest) { res.status(403).json({ error: 'Guests cannot change avatar' }); return; }

    const { preset } = req.body as { preset: string };
    const validColor = /^color:[1-8]$/.test(preset);
    const validImg = /^img:[a-zA-Z0-9_\-]+\.(png|jpg|jpeg|webp)$/.test(preset);
    if (!validColor && !validImg) { res.status(400).json({ error: 'Invalid preset' }); return; }
...
router.put('/me/password', authMiddleware, async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthRequest;
    if (authReq.isGuest) { res.status(403).json({ error: 'Guests cannot change password' }); return; }
    ...
    if (!user || !user.passwordHash) {
      res.status(400).json({ error: 'Password change not available for this account' }); return;
    }
...
router.put('/me/username', authMiddleware, async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthRequest;
    if (authReq.isGuest) { res.status(403).json({ error: 'Guests cannot change username' }); return; }

    const { username } = req.body as { username: string };
    if (!username || username.length < 3 || username.length > 30 || !/^[a-zA-Z0-9_]+$/.test(username)) {
      res.status(400).json({ error: 'Nieprawidłowa nazwa użytkownika (3-30 znaków, litery/cyfry/podkreślnik)' }); return;
    }
    ...
    if (user.lastUsernameChange) {
      ...
      if (elapsed < WEEK_MS) {
        res.status(429).json({ error: 'Zbyt wcześnie', retryAfter: WEEK_MS - elapsed }); return;
      }
    }

    const taken = await User.findOne({ username, _id: { $ne: authReq.userId } });
    if (taken) { res.status(400).json({ error: 'Ta nazwa użytkownika jest już zajęta' }); return; }

    user.username = username;
    user.lastUsernameChange = new Date();
    await user.save();

    res.json({ username: user.username, lastUsernameChange: user.lastUsernameChange });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
});
